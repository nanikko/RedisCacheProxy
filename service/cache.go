...
func (c *ProxyCache) Get(key string) (bool, string) {
	...
	if val,ok := c.lru.Get(key); ok {
		...
		if (time.Now().Unix() - cachedv.lastUpdatedTime) >= c.expiry {
			// expired
			log.Println("[ProxyCache.Get]: Key", key, "expired.")
			c.lru.Remove(key)
			return false, ""
		}

		log.Println("[ProxyCache.Get]: Got non-expired Key", key, "in cache. Refreshed lastUpdatedTime.")
		return ok, cachedv.v
	}
	return false, ""
}
