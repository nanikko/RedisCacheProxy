...
func main() {
	...
	redisHandlerResp, err := infra.NewRedisHandlerResp(*redisAddr)
	if err != nil {
	...
	srv := service.ProxyService{cache, redisHandler, redisHandlerResp, scheduler}
	go srv.Scheduler.Run()

	router := httprouter.New()
	router.GET("/get/:key", srv.GetHandler)
	router.GET("/set/:key/:value", srv.SetHandler)
